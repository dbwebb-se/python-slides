...
class TestAccount(unittest.TestCase):
    ...
    def test_balance_after_deposit2_not_ok(self):
        """ Test it the account balance after deposit with a negative number is the same """
        my_account = Account(10000, "löning", "Marie") # Arrange 
        result = my_account.deposit(-500) # Act
        self.assertFalse(result, "Deposit not allowed, should return false.") # Assert
        self.assertEqual(my_account._balance, 10000, "Balance should be the same, 10000.") # Assert

    def test_transfer_ok(self):
        """ Test that transfer between 2 accounts goes ok. """
        self.skipTest("Test under devopment")